...
- [Test Cases](docs/test_case.md): Manual test plan and expected outcomes
...
This project implements a real-time ETL data pipeline designed to ingest and process user activity from a simulated e-commerce platform. The system captures events such as product views and purchases and the product data, processes them using Spark Structured Streaming, and ingests the processed data into a PostgreSQL database for further analysis and insights.
...
* Measure and evaluate the performance characteristics of the system (to be fully implemented after resolving `spark-submit` issues).
...
A visual representation of this architecture is provided in the ![System Architecture](system_architecture.png).
...
* **Jupyter Notebook:** Used as the current environment to run and develop the Spark streaming application. The goal is to eventually use `spark-submit` for deployment and performance monitoring.
...
5.  Users can then query the PostgreSQL database to analyze the real-time e-commerce activity. Performance monitoring will be implemented once the `spark-submit` issue is resolved.
...
* `performance_metrics.md`: Report with system performance data (latency, throughput, etc.) - to be populated after `spark-submit` is configured.
...
* Data is written to the `user_events` table in PostgreSQL without errors.
* The performance of the system (e.g., processing speed) will be evaluated once the `spark-submit` setup is complete.

## 8. Performance Metrics

The performance of the real-time data pipeline will be evaluated based on metrics such as:

* **Latency:** The time delay between an event being generated and it being written to the PostgreSQL database (to be measured with `spark-submit`).
* **Throughput:** The number of events processed per unit of time (to be measured with `spark-submit`).
* **Resource Utilization:** CPU and memory usage of the Spark cluster and PostgreSQL database (to be monitored with `spark-submit`).
